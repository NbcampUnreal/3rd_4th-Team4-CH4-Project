...
### **발생한 문제**
 ...
 - **발생한 문제 7**
   > 문제점 : 체포 범위 안에 도둑이 있는데도 체포가 안되는 현상 발생
   > 
   > 원인 : findArrestTarget()이 ECC_Pawn 채널만 감지하도록 되어 있어 스윕 이동 시 충돌 보정이 일어나고 서버와 클라이언트 간 위치 오차가 발생
   > 
   > 해결한 방법 : 잠금 유지 중 위치 보정 시 if (HasAuthority())를 사용해 **서버에서만** 좌표 보정하고, 클라이언트는 복제(Replication) 결과만 반영하도록 변경
   > 